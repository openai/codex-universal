...
set +e
nvm_alias_default_output="$(nvm alias --no-colors default 2>/dev/null)"
nvm_alias_default_status=$?
set -e
if [ "${nvm_alias_default_status}" -eq 0 ] && [ -n "${nvm_alias_default_output}" ]; then
